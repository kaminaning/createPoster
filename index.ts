interface posterOptions {
  ...
  bg?: { src: string | URL; pos: Array<number>; width: number; height: number };
  fill?: string;
...
function createPoster({
  ...
  return new Promise(function (resolve, reject) {
    ...
    const handleOpts = () => {
      if (eleList) {
        let flag = 0;
        function drawImg(item: ImageType, i: number, tmpImg: HTMLImageElement) {
          let count = i + 1;
          if (count > flag) {
            setTimeout(() => {
        ...
        function drawText(item: TextType, i: number) {
          let count = i + 1;
          if (count > flag) {
            setTimeout(() => {
              drawText(item, i);
        ...
        eleList.forEach((item, i) => {
          if (item.type === "image") {
            let tmpImg: HTMLImageElement = new Image();
            tmpImg.src = (item as ImageType).src as string;
            tmpImg.crossOrigin = "Anonymous";
            tmpImg.onload = function () {
              if ((this as any).complete) {
                flag++;
                drawImg(item as ImageType, i, tmpImg);
              }
            };
